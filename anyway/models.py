...
class DeprecatedCity(Base):
    ...
    def get_name_from_symbol(symbol: int) -> str:
        res = db.session.query(DeprecatedCity.search_heb).filter(DeprecatedCity.symbol_code == symbol).first()
        if res is None:
    ...
    def get_symbol_from_name(name: str) -> int:
        res = db.session.query(DeprecatedCity.symbol_code).filter(DeprecatedCity.search_heb == name).first()
        if res is None:
